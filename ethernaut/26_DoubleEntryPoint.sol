...
/*
botAddr = new DoubleEntryPointProtectingBot(contract.address)
fortaAddr = await contract.forta()
Forta(fortaAddr).setDetectionBot(botAddr)
*/
contract DoubleEntryPointProtectingBot is IDetectionBot {
    IForta public forta;
    address public vault;

    constructor (DoubleEntryPoint dep) public {
        forta = dep.forta();
        vault = dep.cryptoVault();
    }

    event DelegateTransferChecked(address to, uint256 value, address origSender);

    function handleTransaction(address user, bytes calldata msgData) external override{
        (address to, uint256 value, address origSender) = abi.decode(msgData[4:], (address, uint256, address));
        emit DelegateTransferChecked(to, value, origSender);
        if (vault == origSender) {
